...
The answer applies to the approval as soon as the handler starts, and a second response to the same request is rejected until the chat records the resolution. It is never written into the `useChat` messages, so `sendAutomaticallyWhen` cannot send it to your chat route. `AISDKThreads` keeps the answer with the chat rather than with the runtime, so switching away from an in-memory thread and back still shows the request answered; the answer is retired once the chat reports the outcome. A reload starts from the chat alone, so bring the resumed run back into the chat (for example with `resumeStream`) and have your endpoint refuse a second resume. If the handler throws, the request reopens and `respondToApproval` rejects with the handler's error.
