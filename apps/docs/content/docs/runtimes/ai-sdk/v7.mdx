...
The answer applies to the approval as soon as the handler starts, and a second response to the same request is rejected until the chat records the resolution. It is never written into the `useChat` messages, so `sendAutomaticallyWhen` cannot send it to your chat route. `AISDKThreads` passes the thread's `Chat` as `unstable_hostApprovalOwner`, so the answer is kept with that chat rather than with the runtime and switching away from an in-memory thread and back still shows the request answered; the answer is retired once the chat reports the outcome. A direct `useAISDKRuntime` caller that does not pass an owner keeps the previous behaviour, where the answer lasts only as long as that runtime. A reload starts from the chat alone, so bring the resumed run back into the chat (for example with `resumeStream`) and have your endpoint refuse a second resume. If the handler throws, the request reopens and `respondToApproval` rejects with the handler's error.
