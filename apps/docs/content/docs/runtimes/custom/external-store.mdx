...
A cancellation is not necessarily resumable. Set `canResume: true` only when your
backend retains a checkpoint for the current run, and supply `onResume` to reconnect
to that checkpoint. The composer then shows Resume when its draft is empty.
Adapters without an explicit checkpoint keep the Send/Cancel controls.

`onResume` must continue the same assistant message and tool state. Do not implement
it with `append`, `reload`, or an empty user message. Set `isRunning` while reconnecting
and clear `canResume` when the checkpoint is consumed, expires, or the thread changes.
A rejected callback re-enables the control for retry; report the failure through your
adapter's normal error state. `useComposerResume` also suppresses repeated clicks
while the callback is pending and preserves nonempty composer drafts.

The same opt-in fields are available on `ExternalThread`, `useAISDKRuntime`, and
`useChatRuntime`. Merely providing `onResume` does not advertise a resumable checkpoint.
With a resumable `AssistantChatTransport`, `useChatRuntime` derives availability
from the thread's stored stream ID and reconnects using the AI SDK's `resumeStream`.
See `examples/with-resumable-stream` for an end-to-end example.
