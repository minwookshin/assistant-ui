...
Use `useToolArgsStatus` to distinguish argument streaming from tool execution. The hook returns a `propStatus` map where each top-level key in the args object resolves from `"streaming"` to `"complete"` as the partial JSON arrives, and `allPropsStatus` tracks completion of the full arguments object. Call it inside a tool-call message part context:
...
const toolkit = defineToolkit({
  submitForm: {
    type: "backend",
    render: ({ args }) => {
      const { allPropsStatus, propStatus } = useToolArgsStatus<{
        email: string;
        phone: string;
      }>();

      return (
        <form className="space-y-4" aria-busy={allPropsStatus === "streaming"}>
          <div>
            <input
              type="email"
              value={args.email ?? ""}
...
`allPropsStatus` remains `"streaming"` while the object is partial, even if every
field received so far is complete. Once the object is complete it becomes
`"complete"`, including while the hook's `status` remains `"running"` during tool
execution. A field absent from `propStatus` has not arrived.

The hook uses parser metadata when available and otherwise reads `argsText`.
This includes the AI SDK's `input-available` state, where the arguments have
finished arriving but tool execution may continue. Any non-running lifecycle
settles argument streaming, including when the text is incomplete or invalid;
`"complete"` does not guarantee valid arguments or a successful tool result.
