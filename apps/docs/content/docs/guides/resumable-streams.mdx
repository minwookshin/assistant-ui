...
### Stop and resume in the composer

With the resumable transport above, pressing **Stop generating** retains the
stream ID. While the composer is empty and the thread is idle, the canonical
`Thread` offers **Resume generating**. This reconnects through
`chat.resumeStream()` and continues the existing assistant response without
sending another user message. When the server emits `finish`, storage is cleared
and the composer returns to Send.

Typing a new draft keeps Send available. A transport without resumable storage
does not expose Resume automatically. Custom adapters can opt in with
`canResume` and `onResume`; they own checkpoint expiry and error state. A
cancelled run without a retained checkpoint cannot be resumed. Custom composer
layouts can use `ComposerPrimitive.Resume` or the headless `useComposerResume`
hook to expose the same action.
